...
int MatlabBufferGadget::process(GadgetContainerMessage<IsmrmrdReconData>* m1)
{
    ...
    {
        auto mxrecon = BufferToMatlabStruct(&recon_data->rbit_[i].data_, false, &m1->getObjectPtr()->rbit_[i].data_.headers_);
        mxSetField(reconArray,i,"data",mxrecon);
        if (recon_data->rbit_[i].ref_)
        {
            auto mxref = BufferToMatlabStruct(recon_data->rbit_[i].ref_.get_ptr(), false, &m1->getObjectPtr()->rbit_[i].data_.headers_);
